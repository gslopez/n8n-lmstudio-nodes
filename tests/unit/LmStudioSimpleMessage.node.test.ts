...
const chatResponse = (content: string, extra?: Partial<{ model: string; finish_reason: string }>) => ({
...
describe('LmStudioSimpleMessage', () => {
	...
	describe('execute', () => {
		it('returns text response with metadata', async () => {
			...
			expect(result[0][0].json._metadata).toMatchObject({ model: 'test-model', finish_reason: 'stop' });
		...
		it('parses JSON response when schema is provided', async () => {
			...
			(mock.helpers.httpRequest as jest.Mock).mockResolvedValue(chatResponse('{"name":"Alice"}'));
